образец кода на python с которого переписан Rake.jl, оставим для дебага
...
# ( changed d.belyaev - not original code )
...
#text = sys.stdin.read()
#for term in term_extractor(text.decode("utf-8")):
#    print term.normalized.encode("utf-8"), term.count
...
def separate_words(text, min_word_return_size):
    ...
    """
    #splitter = re.compile(u'[^a-zA-Z0-9_\\+\\-/а-яА-Я]')
    splitter = re.compile(u'[^абвгдеёжзийклмнопрстуфхцчшщьыъэюя\\w\\-\\+]', flags=re.IGNORECASE)
    words = []
    ...
    for s in sentence_list:
        ...
        for phrase in phrases:
            ...
            if phrase != "" and is_acceptable(phrase, min_char_length, max_words_length):
                ...
                #print("APPENDED:",phrase.encode("utf-8"))            
...
#print(phraseList)
#if not phraseList:
#    print("Empty phraseList")

#for p in phraseList:
#    print("PHRASE:"+p.encode("utf-8"))
...
#for w in wordscores:
#    print( "SCORE: "+w.encode("utf-8")+" : "+str(wordscores[w]) )
#exit
...
minchars=int(sys.argv[1])
maxwords=int(sys.argv[2])
minfreq=int(sys.argv[3])
head=int(sys.argv[4])
#print(minchars, maxwords, minfreq) # for debug

rake = Rake( "SmartStoplist.txt", minchars, maxwords, minfreq )
keywords = rake.run(text)
#print(keywords)

#rv = sorted(keywords, key=lambda tu: tu[1])#, reverse=True)#[:5]
rv = keywords
for kv in rv[:head]:
    print( "--(", kv[0].encode("utf-8"),",",kv[1],")--") # debug version of print
    #print( kv[0].encode("utf-8") ) # only text
